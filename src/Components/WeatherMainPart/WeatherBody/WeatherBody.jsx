        ...
        <div>
            {
                props.data && props.data['weather'] ?<div className={css.CurrentDay}>
                    <div className={css.Container}><img className={css.Image} src={`http://openweathermap.org/img/wn/${props.data['weather'][0].icon}@2x.png`} alt={props.data['weather'][0].icon}/></div>
                    <div className={css.WeatherText}>{Math.round(props.data['main'].temp - 273.15)}<sup>&deg;C</sup></div>
                ...
                </div>: Loading ?<div style={{display: 'flex' , justifyContent: 'center'}}><Spinner  style={{width:'50px', height:'50px'}} animation="border" /></div>
